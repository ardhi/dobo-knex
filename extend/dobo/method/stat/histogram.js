...
async function statHistogram ({ schema, filter = {}, options = {} }) {
  ...
  try {
    let file = getPluginFile(`${this.ns}:/extend/dobo/lib/${driver.type}/stat-histogram/${options.type}.js`)
    if (!fs.existsSync(file)) file = getPluginFile(`${this.ns}:/extend/dobo/method/stat/histogram/${options.type}.js`)
