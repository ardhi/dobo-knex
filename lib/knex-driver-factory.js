...
async function knexDriverFactory () {
  ...
  const { omit, has, forOwn, cloneDeep, isEmpty } = this.app.lib._
  ...
  class KnexDriver extends Driver {
    ...
    async createRecord (model, body = {}, options = {}) {
      const client = model.connection.client
      const result = await client(model.collName).insert(body, this._getReturningFields)
    ...
    async createHistogram (model, filter = {}, options = {}) {
      ...
      this._reformHistogram({ item, type, group, aggs, field })
